...
const adminSections: NavigationSection[] = [
  ...
  { title: "Administración", items: [
    { key: "copropietarios", label: "Copropietarios", href: "/copropietarios", icon: UsersIcon },
    { key: "inquilinos", label: "Inquilinos", href: "/inquilinos", icon: HomeIcon },
    { key: "departamentos", label: "Departamentos", href: "/departamentos", icon: BuildingIcon },
    { key: "espacios", label: "Parqueos y bauleras", href: "/espacios", icon: BuildingIcon },
  ...
  { title: "Seguridad", items: [
    { key: "seguridad", label: "Seguridad y auditoría", href: "/seguridad", icon: ShieldIcon },
  ] },
  { title: "Sistema", items: [
    { key: "respaldo", label: "Nube y respaldos", href: "/respaldo", icon: SettingsIcon },
    { key: "usuarios-roles", label: "Usuarios y roles", href: "/usuarios-roles", icon: UsersIcon },
  ] },
];

const ownerSections: NavigationSection[] = [
  { title: "Principal", items: [{ key: "dashboard", label: "Dashboard", href: "/dashboard", icon: LayoutIcon }] },
  { title: "Mi vivienda", items: [{ key: "departamentos", label: "Mi departamento", href: "/dashboard", icon: BuildingIcon }] },
  { title: "Finanzas", items: [{ key: "finanzas", label: "Expensas y finanzas", href: "/finanzas", icon: WalletIcon }] },
  { title: "Operaciones", items: [{ key: "mantenimiento", label: "Mantenimiento", href: "/mantenimiento", icon: WrenchIcon }] },
  { title: "Documentos", items: [{ key: "documentos", label: "Gestión documental", href: "/documentos", icon: FileIcon }] },
  { title: "Seguridad", items: [{ key: "seguridad", label: "Seguridad y auditoría", href: "/seguridad", icon: ShieldIcon }] },
];
...
const consultationSections: NavigationSection[] = [
  ...
  { title: "Consulta", items: [
    { key: "departamentos", label: "Departamentos", href: "/departamentos", icon: BuildingIcon },
    { key: "documentos", label: "Gestión documental", href: "/documentos", icon: FileIcon },
  ] },
];
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobile ? "open" : ""}`}>
        <div className="sidebar-brand">
          <span className="brand-mark"><BuildingIcon size={21} /></span>
          <span>Edificio <strong>Central</strong></span>
        </div>
      ...
      {mobile && <button className="mobile-overlay" onClick={() => setMobile(false)} aria-label="Cerrar menú" />}
      ...
      <div className="main-shell">
        <header className="topbar">
          ...
          <div className="breadcrumb">
            <span>Edificio Central</span><b>/</b><strong>{currentTitle}</strong>
          </div>
          <div className="topbar-actions">
            <button className="top-icon" aria-label="Notificaciones"><BellIcon size={19} /><i /></button>
            <div className="top-user">
              <div className="avatar">{initials(name)}</div>
              <div><strong>{name}</strong><span>{roleLabel}</span></div>
            </div>
            <button className="btn btn-secondary top-logout" onClick={logout}>Cerrar sesión</button>
          </div>
        </header>
