    ...
    <DashboardLayout active="usuarios-roles">
      <div className="page-heading">
        <div>
          ...
          <p>
            Administra los usuarios del sistema y los roles que determinan sus
            permisos.
          </p>
        </div>
      </div>
      ...
      <section className="panel usuarios-roles-card">
        <div className="usuarios-roles-icon">
          <UsersIcon size={30} />
        </div>
        <div className="usuarios-roles-copy">
          ...
          <p>
            Desde este módulo puedes asignar roles, eliminar usuarios y
            registrar automáticamente los cambios en auditoría.
          </p>
          <div className="hu48-role-chips">
            {(["ADMINISTRADOR", "DIRECTORIO", "CONSULTA"] as const).map(
              (role) => (
                <span key={role}>
        ...
        <button
          className="btn btn-primary usuarios-roles-open"
          onClick={openManager}
        >
      ...
      <section className="panel">
        <div className="panel-heading">
          <div>
            ...
            <p>
              El administrador es el único rol autorizado para modificar estos
              permisos.
            </p>
          </div>
        </div>
        <div className="hu48-summary">
          <div className="hu48-info">
            ...
            <div>
              ...
              <span>
                Los cambios de rol y eliminaciones quedan registrados con fecha
                y hora.
              </span>
            </div>
          </div>
    ...
    <Modal open={open} title="Gestionar roles" onClose={onClose}>
      <div className="hu48-modal-body">
        <div className="hu48-modal-intro">
          <div>
            <p>
              Selecciona el rol que determinará los permisos de cada usuario.
              Los cambios se registran automáticamente en auditoría.
            </p>
          </div>
        </div>
        <div className="hu48-role-table">
          ...
          {users.map((user) => (
            <div className="hu48-role-row" key={user.id}>
              ...
              <select
                value={
                  adminRoles.includes(user.role as ManagedRole)
                    ? user.role
                    : "CONSULTA"
                }
                disabled={savingId === user.id || deletingId === user.id}
                onChange={(event) =>
                  changeRole(user.id, event.target.value as ManagedRole)
                }
        ...
        {message && (
          <div className="alert alert-success hu48-message">{message}</div>
        )}
        <div className="hu48-audit-note">
          ...
          <div>
            ...
            <span>
              Cada cambio guarda usuario, fecha, rol anterior y nuevo rol.
            </span>
          </div>
        </div>
