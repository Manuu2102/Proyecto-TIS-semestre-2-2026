...
export class OcupacionesService {
  ...
  async crear(data: CrearOcupacionDto) {
    const usuario = await this.prisma.usuario.findUnique({
      where: { id: BigInt(data.id_copropietario) },
    });
    if (!usuario) {
    ...
    const existe = await this.prisma.departamento_usuario.findFirst({
      where: {
        id_copropietario: BigInt(data.id_copropietario),
  ...
  async historialPorDepartamento(id_departamento: bigint) {
    ...
    return {
      ...
      historial: historial.map((h) => ({
        ...
        usuario: {
          ...h.usuario,
          id: h.usuario.id.toString(),
          ci: h.usuario.ci.toString(),
          id_user: h.usuario.id_user.toString(),
        },
      })),
    };
  }

  async cerrar(id_copropietario: bigint, id_departamento: bigint) {
  ...
  private formatear(o: any) {
    return {
      ...o,
      id_copropietario: o.id_copropietario.toString(),
